...
import type { TrainingDraft } from './shared'

export function TrainingSetup({ options, draft, setDraft, repository, setRepository, busy, preparing, onPrepare, onStart }: {
  ...
  return <form onSubmit={onStart} className="grid items-start gap-8 lg:grid-cols-[minmax(0,1fr)_300px]">
    <div className="space-y-8">
      ...
      <details className="border-border-subtle rounded border p-4"><summary className="cursor-pointer text-sm font-medium">Advanced settings · Layer {draft.layer}</summary><p className="text-text-secondary mt-3 text-xs">The default layer is a starting point. Change it to investigate a specific part of the model.</p><div className="mt-4 grid gap-4 sm:grid-cols-2">
        <label className="text-text-secondary text-sm">Layer<input className={field} type="number" min="0" max={(options?.layers ?? 26) - 1} required value={draft.layer} onChange={(event) => update('layer', Number(event.target.value))} /></label>
    ...
    <aside className="border-border-subtle bg-bg-surface rounded border p-5 lg:sticky lg:top-6"><h3 className="font-medium">Your training plan</h3><dl className="mt-5 space-y-4 text-sm">{[['Model', repository], ['Text', draft.dataset === 'text' ? 'Your text' : 'Sample stories'], ['Layer', String(draft.layer)], ['Features', number(draft.features, 0)], ['Training tokens', number(draft.training_tokens, 0)], ['Compute', options?.device ?? 'Connecting…']].map(([label, value]) => <div className="flex justify-between gap-4" key={label}><dt className="text-text-secondary shrink-0">{label}</dt><dd className="break-all text-right">{value}</dd></div>)}</dl><p className="text-text-secondary mt-5 text-xs leading-5">{options?.estimate_note ?? 'Runtime estimate unavailable.'} Evaluation follows training.</p><p className="text-text-tertiary mt-3 text-xs leading-5">Training and exploration share one compute queue.</p>
      {ready ? <button className={`${primary} mt-5 w-full`} type="submit" disabled={!!busy}>{busy ?? 'Start training'}</button> : <button className={`${primary} mt-5 w-full`} type="button" disabled={!options || !!busy || preparing} onClick={onPrepare}>{busy ?? (preparing ? 'Preparing model…' : 'Prepare model')}</button>}
